...
WildProxyListener.prototype.__onProxyError = __onProxyError;
...
function __onProxyError(err, req, res){
  console.warn(`proxy error occurred for url: ${req.url}, error: ${err.message}`);
  res.writeHead(500);
  res.end();
}
...
function listen () {
  ...
  return new Promise(function (resolve, reject) {
    ...
    try {
      ...
      _this.__proxy.on('error', _this.__onProxyError.bind(_this));
